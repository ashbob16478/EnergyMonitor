...
local ab=da("utils")local bb=ab.tableCount
return
{VisualObject=function(cb,db)local _c={}local ac={}
local bc={x="getX",y="getY",w="getWidth",h="getHeight"}
local function cc(bd)
local cd,dd=pcall(load("return "..bd,"",nil,{math=math}))if not(cd)then
...
bd:addBG(1,i,_d[3][i])bd:addText(1,i,_d[1][i])end end end)end}return ad end}end;aa["libraries"]={}
...
aa["objects"]["Menubar"]=function(...)local ab=da("utils")
local bb=da("tHex")
return
function(cb,db)local _c=db.getObject("List")(cb,db)local ac="Menubar"local bc={}
_c:setSize(30,1)_c:setZIndex(5)local cc=0;local dc,_d=1,1;local ad=true
local function bd()local cd=0;local dd=_c:getWidth()
local __a=_c:getAll()
for n=1,#__a do cd=cd+__a[n].text:len()+dc*2 end;return math.max(cd-dd,0)end
...
db.customEventHandler(d_a,_aa,...)if _aa=="basalt_FrameResize"then bd=true end end,removeChild=function(d_a,_aa)
if(
type(_aa)=="string")then _aa=getChild(element:getName())end;if(_aa==nil)then return end;db.removeChild(d_a,_aa)for aaa,baa in pairs(_d)do
if
baa:getName()==_aa:getName()then table.remove(_d,aaa)break end end;bd=true
d_a:updateDraw()return d_a end,draw=function(d_a)
db.draw(d_a)
...
if
aca.element:getName()==dba:getName()then table.remove(bc,_ca)return true end end
cba:removeEvents(dba)ad=false end;local function aaa(cba)local dba=cba:getParent()bc={}cc={}ad=false;bd=0;cd=0;_d=nil
...
#bc,d_a)),ab[cc]:rep(math.max(#bc,d_a)))end end end end end)end}a_a.__index=a_a;return setmetatable(a_a,db)end end;return aa["main"]()